...
class TestPolicyMerge:
    ...
    def test_local_overrides_global(self, tmp_path):
        ...
        assert merged["max_tasks"] == 5  # local wins
    ...
    def test_absent_local_keys_fall_through(self, tmp_path):
        ...
        merged = merge_policy(proj)
        assert merged["key_a"] == 1  # from global
        assert merged["key_b"] == 99  # from local
    ...
    def test_missing_local_uses_all_global(self, tmp_path):
        ...
        ensure_global_dirs()
        write_json(global_policy_path(), {"poll_interval": 600, "enabled": True})
        proj = _make_project(tmp_path, "proj-a")
        # No local policy file at all

        merged = merge_policy(proj)
        assert merged["poll_interval"] == 600
    ...
    def test_corrupt_local_ignored(self, tmp_path):
        ...
        merged = merge_policy(proj)
        assert merged["from_global"] is True
...
class TestIsolation:
    ...
    def test_merge_policy_does_not_cross_project_boundaries(self, tmp_path):
        ...
        assert merged_a["key"] == "from-a"
        assert merged_b["key"] == "from-b"
